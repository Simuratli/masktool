...
function TableRow({ mask, name, rule, searchName, rowName, mainName, value, attributeTypeCode }: TableRowPropTypes) {
    ...
    useEffect(() => {
        ...
        let newMaskingRule = changeMaskingRule(searchName, defaultTasksState, viewsByEntityState, rowName, name, ruleNew, mainName)
        if (newMaskingRule.for === "entities") {
            dispatch(setDefaultTasks(newMaskingRule.data))
        } else {
            dispatch(setAllViewsByEntity(newMaskingRule.data))
                ...
                if (view.name === mainName) {
                    view.data.map(async (item) => {
                        if (item.name === rowName) {
                            let newCells = await item.cells.filter((data) => data.displayName !== name)
