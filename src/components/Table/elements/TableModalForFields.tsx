...
function TableModalForFields() {
    ...
    const confirm = useCallback(
        () => {
            ...
            console.log(data, 'offfffffffffffffffffffffffff')
            console.log(viewsByEntityState, 'offfffffffffffffffffffffffff viewsByEntityState')
            console.log(defaultTasksState, 'offfffffffffffffffffffffffff defaultTasksState')
        },
        [stableDataReducer, defaultTasksState, selectedFields, viewsByEntityState, dispatch],
    )
    ...
    useEffect(() => {
        ...
        if (stableDataReducer.searchName === 'entities') {
            ...
            console.log(stableDataReducer, 'haloooooo sabledata')
            stableDataReducer.entities.map((entity) => {
                if (entity.name === stableDataReducer.name) {
                 
                    if (stableDataReducer.mainName) {
                        console.log(stableDataReducer, 'viewsByEntityState.entities', viewsByEntityState.entities)

                        var index = viewsByEntityState.entities.findIndex(function (item, i) {
                            return item.name.toLowerCase() === (typeof stableDataReducer.mainName === 'string' && stableDataReducer.mainName.toLowerCase()) || item.name.toLowerCase() === (typeof stableDataReducer.name === 'string' && stableDataReducer.name.toLowerCase()) || item.name.toLowerCase() === (typeof stableDataReducer.logicalName === 'string' && stableDataReducer.logicalName.toLowerCase())
                        });

                        console.log(index, 'haloooooo index')
                        let dataNew = []
                        if (index >= 0) {
                            ...
                            console.log(secondIndex, 'haloooooo secondIndex')
                            console.log(entity.data, 'haloooooo entity.data ')
                            console.log(viewsByEntityState.entities[index].data[secondIndex], 'haloooooo viewsByEntityState.entities[index].data[secondIndex]')
                            dataNew = getDifference((entity.data ? entity.data : []), viewsByEntityState.entities[index].data[secondIndex].cells)

                        }

                        console.log(dataNew, 'haloooooo datanew')
                        setdataFields(dataNew)
                        setsearchAbleData(dataNew)
    ...
    }, [defaultTasksState.tasks, stableDataReducer, modalState.addFields])
    ...
    return (
        <Modal>
            ...
            <div style={{ height: 250 }} className='modal__scroll__container'>
                {console.log(searchAbleData, 'varmi')}
